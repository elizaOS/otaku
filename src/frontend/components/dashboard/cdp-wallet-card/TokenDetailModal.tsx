...
export function TokenDetailModal({ token, onClose }: TokenDetailModalProps) {
  ...
  // Calculate current price from USD value and balance
  const currentPrice = token.balanceFormatted && parseFloat(token.balanceFormatted) > 0
    ? token.usdValue / parseFloat(token.balanceFormatted)
    : 0;
    ...
    const fetchPriceHistory = async () => {
      ...
      try {
        ...
        if (!token.contractAddress) {
          ...
          const platformMap: Record<ChainNetwork, string> = {
            base: 'base',
            ethereum: 'ethereum',
            polygon: 'polygon-pos',
          };
